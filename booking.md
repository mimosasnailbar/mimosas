---
layout: default
---
<section class="wrapper home__main">
    <div id="imageToggle">
        <div class="home__image" v-bind:style="{ backgroundImage: `url('${src}')` }">
            <h2 style="text-align: center; font-weight: bold; color: black;">  Book Here </h2>
            <div style="text-align: center; class="home__buttons><a  href="https://www.vagaro.com/us03/Users/BusinessWidget.aspx?enc=" class="btn-booking" target="_blank">Mimosas Nail Bar 2320 North Davidson Street, 2, Charlotte, NC, 28205</a></div><br>
            <h2 style="text-align: center; font-weight: bold; color: black;">  Book Here </h2>
            <div style="text-align: center; class="home__buttons><a href="https://www.vagaro.com/us03/Users/BusinessWidget.aspx?enc=" class="btn-booking" target="_blank">Mimosas Nail Bar- Metropolitan 111 Metropolitan Avenue, Suite 100, Charlotte, NC, 28204</a></div><br>
            <h2 style="text-align: center; font-weight: bold; color: black;">  Book Here  </h2>
           <div  style="text-align: center;class="home__buttons><a href="https://mimosasnailbar3715.simplepos.us/" class="btn-booking" target="_blank">Mimosas Nail Bar 9695 Red Stone Drive, Indian Land, SC, 29707</a>
            </div><br>
        </div>
    </div>
</section>
<style>@media only screen and (max-width: 600px) {
    .home__main .home__image {
        height: 100%;
    }
</style>
